...
namespace MyBooks.Services
{
    public class MetadataService
    {
        ...
        private async Task<BookDto> ReadEpubMetadata(string epubPath)
        {
            ...
            string? dateString = book.Schema?.Package?.Metadata?.Dates?.FirstOrDefault()?.Date;

            if (DateTime.TryParse(dateString, out var publishedDate))
            {
                if (publishedDate.Year >= 1753)
                {
            ...
            {
                meta.CoverPath = Convert.ToBase64String(book.CoverImage);
            }
